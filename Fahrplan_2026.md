...
| **B21** Tarif- & Ladeoptimierung | **teilweise gebaut** | Schema (B21-1) + Schreibweg vollständig (B21-2a aWATTar-Sync, B21-2b Admin-Pflege) + Datenzugriff im Rechner samt Regel B (B21-3a) — offen bleiben die Engine-Preislogik (B21-3b) und die Oberfläche. Kanonische Quelle für die fachliche Tiefe: Pflichtenheft_Kalkulator_Delta_Tarifoptimierung.md |
...
- **B17** Admin-Portal — **GEBAUT (27.07.2026), einziger Schritt, kein Folgeschritt geplant.** Der Verwaltungsbereich bekommt einen eigenen Anmelde-Eingang (`/admin/anmelden`) und einen eigenen Rahmen (Kopfzeile mit COOLiN-Logo, Kennzeichnung „Admin-Bereich", angemeldetes Konto, Abmelden, Navigation über die fünf bestehenden Bereiche). **Keine Migration, keine Änderung an einem `admin_*`-Wrapper und an keinem Grant** — reine Anwendungsarbeit in `apps/web`; `apps/website`, `packages/**` und `supabase/` mit 0 Zeilen Diff, ebenso `/anmelden`, `/konto`, `/registrieren`, `/partner-werden` und `/partner-portal`. 10 neue Anwendungstests (266 → **276**), DB-Gate unverändert.

  **Die tragende Entscheidung: es entsteht KEIN zweites Authentifizierungssystem.** Derselbe Supabase-Auth-Bestand, dieselbe Sitzung, dieselbe Server Action (`signInAction` aus T4-2) und dieselbe Rollenprüfung aus T4 (`platform.user_roles` über `public.is_admin`). Neu sind ausschliesslich ein eigener **Eingang** und ein eigener **Rahmen**. **Eine eigene Anmelde-URL erhöht die Sicherheit nicht — sie schafft Klarheit; der Schutz liegt unverändert in der Rollenprüfung.** Konkreter Nutzen: Wer sich dort anmeldet, landet im Verwaltungsbereich statt auf der Kontoseite, und wer sich abmeldet, kommt an denselben Eingang zurück statt auf die Marketing-Startseite.

  **Die Rollenvergabe bleibt dauerhaft manuell in Supabase — ausdrücklich keine Oberfläche dafür.** Das ist eine Festlegung, keine Vertagung: es gibt bewusst keinen Weg über die Oberfläche, Admin zu werden, auch nicht für bestehende Admins. Der Eingang trägt folgerichtig keinen Verweis auf Registrierung, Partnerprogramm oder Passwort-Zurücksetzen; er ist `noindex, nofollow` und steht in keiner sitemap (die Routenliste in `lib/routes.ts` liest ausschliesslich `app/(site)/[locale]/` — `/admin` lag noch nie darin).

  **Ein Konto ohne Admin-Rolle bekommt am Eingang keine Sonderbehandlung.** Die Seite prüft gar keine Rolle: eine bestehende Sitzung wird ohne Ansehen auf `/admin` weitergeleitet, und dort entscheidet der unveränderte Guard — neutrale „Kein Zugriff"-Seite, neutraler Seitentitel, kein Hinweis darauf, dass eine Rolle fehlt oder dass es hier etwas zu haben gäbe.
...
- **B21** Tarif- & Ladeoptimierung — der Kalkulator bekommt einen **zweiten, unabhängigen Ersparnis-Hebel** neben Peak Shaving: Ersparnis durch optimalen Stromtarif und optimales Ladeverhalten, simuliert gegen **echte historische Marktpreise** (aWATTar), in Kombination mit der passenden Batterie. Damit wird Peak Shaving optional statt tragende Säule — ein Kunde ohne Leistungspreis-Komponente (Privathaushalt, Netzebene ohne Leistungsmessung) bekommt trotzdem eine vollständige, ehrliche Analyse. Kanonische Quelle für die fachliche Tiefe ist `Pflichtenheft_Kalkulator_Delta_Tarifoptimierung.md`; für Reihenfolge und Umfang bleibt diese Datei maßgeblich.
  ...
  **ERLEDIGT: B21-2a (27.08.2026, SCHREIBWEG aWATTar).** Die Spotpreise füllen sich jetzt selbst. Migration `supabase/migrations/20260827160000_grant_service_role_spot_prices_write.sql` (genau ein Grant), die reine Sync-Logik `apps/web/lib/spot-prices/sync.ts`, der dritte Cron-Job `apps/web/app/api/cron/spot-price-sync` (vorgesehen täglich **13:20 UTC**, **noch nicht in `vercel.json` registriert** — s. u.) und das einmalige Backfill-Skript `apps/web/scripts/backfill-spot-prices.mjs`. 23 neue Tests in `apps/web` (587 → **610**), 11 netto neue im DB-Gate (643 → **654**). **Keine Engine-Änderung, keine UI-Änderung, kein Tarif-Pflege-UI** — `packages/engine`, `apps/website` und die beiden Tabellen `grid_tariffs`/`grid_tariff_rate_windows` sind unangetastet.

  **ERLEDIGT: B21-2b (28.08.2026, SCHREIBWEG ADMIN-PFLEGE). Der Schreibweg ist damit vollständig — offen bleibt nur noch die Oberfläche.** Die Netzbetreiber-Tarife lassen sich jetzt pflegen. Migration `supabase/migrations/20260828090000_create_grid_tariff_write_path.sql` (die Datenbankfunktion `public.create_grid_tariff` + die zwei Grants), der Schreibpfad `apps/web/lib/admin/grid-tariffs-actions.ts` und die Admin-Seite `/admin/netzbetreiber-tarife`. 20 neue Tests im DB-Gate (654 → **674**). **Keine Engine-Änderung, keine Änderung an `apps/website`, keine Änderung an `spot_prices` oder am aWATTar-Cron** — und ausdrücklich KEINE Stromanbieter-Tarife (`retail_tariffs`): Delta 5 nennt beide Seiten gemeinsam, für diesen Bauabschnitt ist entschieden, nur die Netzbetreiber-Seite zu bauen.

  **Die Entscheidung, die B21-2b trägt: eine Datenbankfunktion, weil ein Tarifstand aus DREI Schreibvorgängen entsteht.** Über PostgREST ist jeder `.insert()`/`.update()` seine eigene Transaktion; bräche der dritte ab, bliebe eine Tarifzeile OHNE Zeitfenster stehen — bei bereits geschlossener Vorgängerin. „Keine Berechnungsgrundlage" ist ein sicherer Zustand (B21-1), „eine halbe" ist es nicht. `public.create_grid_tariff` klammert deshalb alles in EINEN Aufruf: die offene Zeile derselben Kombination wird auf `valid_from − 1 Tag` geschlossen, die neue angelegt, ihre Zeitfenster dazu.

  **Nur Anhängen, kein Bearbeiten, kein Löschen.** Ein neuer Stand muss NACH dem Beginn des offenen liegen; eine rückwirkende Korrektur bleibt bewusst ein seltener Eingriff von Hand. Es gibt weder ein `delete`-Grant noch eine Update-Funktion — dieselbe Haltung wie beim Append-only der Analyse-Ablage (B14-1): eine 2026 archivierte Analyse muss 2028 noch sagen können, welcher Stand ihr zugrunde lag.

  **⚠ Die Autorisierung liegt hier im Anwendungscode, und das ist die einzige Stelle des Systems, an der das so ist.** `create_grid_tariff` ist SECURITY INVOKER und läuft als `service_role` — die trägt kein JWT, `auth.uid()` ist leer, es gibt in der Datenbank nichts zu prüfen. Die Zugangsentscheidung fällt in der Server Action (`isCurrentUserAdmin()`, fail-closed) und im Admin-Layout; die ESLint-Erlaubnisliste für den service_role-Client ist um genau diese eine Datei erweitert. Ein Schreib-Grant für `authenticated` wäre die Alternative gewesen und gälte für jedes angemeldete Konto — einfangen liesse sich das nur mit einer RLS-Policy über `platform.is_admin()`, also mit genau dem Wrapper-Muster, das B21-1 für diese Tabellen verworfen hat. Vollständig begründet in `DEPLOYMENT.md` §3c.

  **ERLEDIGT: B21-3a (28.08.2026, ENGINE TEIL 3a — DATENZUGRIFF + REGEL B). Der Kalkulator hat zum ersten Mal eine Datenbank-Anbindung.** `apps/website/lib/tariff-data/` liest die Referenzdaten als `anon`, ausschliesslich lesend: `fetchGridTariffs` (alle Tarifzeilen samt Zeitfenstern, deren Gültigkeit den Lastgang-Zeitraum überschneidet — nicht nur eine, denn ein Jahreslastgang überquert Tarifwechsel) und `fetchSpotPrices` (alle Preise im Bereich, samt `complete: boolean` und der Liste fehlender Bereiche — die Datengrundlage für Delta 15 Regel C). Dazu **Regel B**: ein Lastgang, der vor dem 1.1.2025 beginnt, wird beim Upload abgelehnt, mit dem konkreten Datum der Datei und dem konkreten frühesten zulässigen. Neue Umgebungsvariablen `NEXT_PUBLIC_SUPABASE_URL`/`NEXT_PUBLIC_SUPABASE_ANON_KEY` im Projekt `peak-shaving-website` (`DEPLOYMENT.md` §1-Website). 11 neue Tests in `packages/shared` (56 → **67**). **Keine Engine-Preislogik** (`intervalTariffRates()` unverändert, das ist B21-3b), **keine UI-Änderung ausser der Ablehnung**, **kein Schreibzugriff auf irgendeine Tabelle**.

  **Zwei Entscheidungen, die B21-3a trägt.** (1) **Regel B sitzt im Upload-Schritt, nicht im Parser** (Delta 14 Punkt 8, damit geschlossen): der Parser beantwortet „lässt sich die Datei lesen?", nicht „wollen wir mit dem Zeitraum rechnen?" — und vier Engine-Testdateien mit 2023er-Fixtures bleiben dadurch unberührt. `parseLoadProfile` trägt einen Kommentar, der die fehlende Prüfung als Entscheidung ausweist. (2) **Regel B prüft den Kalendertag in der Zeitzone des Lastgangs, nicht den UTC-Zeitpunkt:** ein österreichischer Kalenderjahr-2025-Export beginnt ortszeitlich am 1.1.2025, in UTC aber eine Stunde davor — gegen den Zeitpunkt geprüft würde ausgerechnet der Regelfall abgewiesen. Die dadurch offene Ein-Stunden-Kante am Jahresanfang ist ausdrücklich als Aufgabe für B21-3b vermerkt.

  **Der Befund, der die Abfrage rettet: PostgREST deckelt eine Antwort ohne Bereichsangabe bei 1.000 Zeilen.** Ein Jahres-Lastgang braucht 8.760 Stundenpreise. Ohne Seitenwechsel gemessen: 1.000 Zeilen und eine **erfundene 50-Tage-Lücke** — der Vergleich wäre „nicht berechenbar", und der Grund stünde nirgends. `fetchSpotPrices` liest deshalb in Seiten.

  **Die Tabellen bleiben leer.** Was fehlt, sind die Preisblätter selbst — und für Netzebene 7 ab 2027 die noch nicht erlassene Tarifverordnung (SNE-T-V).
