...
- **Zahlungs-Entscheidungen (in T4-3 real gebaut):**
  - **Gehostetes Stripe-Checkout, kein Stripe.js.** Checkout- und Portal-Start laufen als Server Actions, die auf eine von Stripe gehostete URL weiterleiten; es wird KEIN Stripe.js/Elements ins Client-Bundle eingebunden (die T3/T4-2-Eigenschaft „`.next/static` = 0 Secrets" gilt jetzt für Supabase UND Stripe). Der Preis lebt ausschließlich in einer Env-Variable (Price-ID) und wird nur von Stripes Checkout-Seite angezeigt — nie im Code/in `de.json` (kommerziell offen, §12 #1).
  - **Customer Portal statt Eigenbau.** Kündigung, Zahlungsmittel und Rechnungen laufen im von Stripe gehosteten Customer Portal (Server Action → Portal-URL), nicht selbst nachgebaut. Die Portal-Konfiguration liegt im Stripe-Account (per API angelegt).
  - **Webhook schreibt `platform` NUR über `public`-RPC-Wrapper, nicht direkt.** `platform` ist nicht über die REST-API erreichbar (T4-1), und es soll genau einen Zugriffsweg geben (T4-2 J3) — der Webhook (service_role) ruft SECURITY-DEFINER-`public`-Wrapper auf, die Event-Aufzeichnung (Idempotenz), Nutzer↔Customer-Verankerung und Subscription-Spiegel ATOMAR bündeln; die Entitlement-Zeile leitet weiterhin der DB-Trigger ab (I2). Die Nutzer↔Customer-Zuordnung wird VOR dem Checkout hergestellt (metadata `user_id`/`product` + `client_reference_id`), nicht danach aus einer E-Mail erraten. Stripe-API-Version explizit gepinnt (`current_period_end` vom SubscriptionItem); eine aktive Stripe-Zeile ohne Periodenende scheitert LAUT (kein Dauerzugang, T4-2 Fail-open-Constraint).
...
- **Realer Cloud-Stand (seit der Deploy-Runde):** Das Projekt **`coolin_energy`** (Org „CoolIn", **eu-central-1**) ist mit dem Repo verknüpft; **alle 5 Migrationen sind gepusht.** Der Cloud-Zustand wurde per Introspektion **bitgenau gegen den lokalen Stand verifiziert** (Schemas/Tabellen+RLS/Enums/Funktionen mit `search_path=''`/Grants/Policies/Trigger/Constraints identisch), das **DB-Gate lief 21/21 grün gegen die Cloud**, und die Platzhalter-Tarife sind eingespielt (11 Snapshots / 10 `current_tariffs`). **Was `supabase db push` NICHT überträgt** (Dashboard-Aktion, s. `DEPLOYMENT.md` im Repo-Root): die exponierten API-Schemas (`monitor` muss im Dashboard freigeschaltet werden, sonst 404 auf `/strom-check`; `platform` bleibt korrekt draußen), Auth-Redirect-URLs und produktionstauglicher SMTP (Resend). Der **`SUPABASE_SERVICE_ROLE_KEY`** ist seit **T4-3** in `apps/web` verdrahtet — server-only, ausschließlich im Stripe-Pfad (Webhook + Checkout/Portal-Actions, umgeht RLS), abgesichert per `import 'server-only'` + ESLint-`no-restricted-imports`.
- **Sicherheits-Messung (Segfault-Probe, Deploy-Runde):** Der aus T4-2 bekannte Postgres-Segfault (Nicht-Owner ruft ungegrantete `public`-Funktion) ist über die **öffentliche REST-API mit dem anon-Key NICHT auslösbar** — der Aufruf liefert sauber `permission denied` (HTTP 401), die Cloud-DB bleibt normal. **Kein DoS über die öffentliche API.** Der Bug ist ausschließlich über eine direkte `SET ROLE`-DB-Verbindung erreichbar (die ein anon-Angreifer nicht hat). Details/Historie: `packages/tariff-monitor/CLAUDE.md` (Handover).
...
| 11 | `[T4-3]` Der Stripe-Account ist ein **fremder Test-Account**, nicht CoolIns eigener. Keys (`sk_test`/`whsec`), Produkt und Price-ID sind vor dem Livegang durch CoolIns echten Stripe-Account zu ersetzen. | Andreas/Martin | Livegang — sonst liefe die Zahlung über ein fremdes Konto. |
| 12 | `[OFFEN-KOMMERZIELL, T4-3]` Der Preis **4,90 €/Monat ist ein Platzhalter** (§2/§12 #1). Im Code nirgends hartkodiert (nur die Price-ID in einer Env-Variable) — ein Preiswechsel ist eine Stripe-/Env-Änderung ohne Umbau. | Andreas/Martin | **Nichts am Bau.** |
| 13 | `[RECHT, T4-3]` **Umsatzsteuer:** der Preis ist derzeit als Bruttopreis angelegt (B2C-üblich in Österreich), Stripe Tax ist AUS. Vor dem Livegang mit dem Steuerberater klären, ob das für ein B2C-Abo in Österreich so bleibt. Reine Stripe-Einstellung, kein Code. | Recht/Steuerberater | Livegang. |
...
4. **T4 — Auth + Stripe + Entitlements** (§4.3): erstes echtes Portal-Fundament (geteilt mit künftigem Kalkulator-Portal). `hasActiveMonitor` aus Stripe-Webhook. In **vier Teile** geschnitten:
   ...
   - **T4-3 Stripe — ABGESCHLOSSEN:** gehostetes Checkout (Server Action → Stripe-Checkout-Seite) + Webhook-Handler (`app/api/stripe/webhook`) + Customer Portal + Abo-Teaser/Konto-Flows. Der Handler schreibt `platform` NICHT direkt, sondern über SECURITY-DEFINER-`public`-RPC-Wrapper (service_role-only), die Event-Aufzeichnung/Customer-Verankerung/Subscription-Upsert ATOMAR bündeln; die Entitlement-Zeile leitet der T4-1-DB-Trigger ab, nicht der Handler. Stripe-API-Version explizit gepinnt (`current_period_end` vom SubscriptionItem). TEST-Modus, fremder Test-Account (§12 #S). Verifiziert gegen `stripe listen` (Kauf mit Testkarte 4242, Idempotenz, Signatur, Out-of-order, fehlendes Periodenende, Portal-Kündigung, K10-Wartezustand) — Details: `packages/tariff-monitor/CLAUDE.md`.
