...
- **[GESAMTSTATUS]** Committed: Prompt 0, 1, Contract-Härtung, U1, Prompt 2 (Parser) + react-hooks-Fix + CI-Fix (pnpm-Version aus dem Workflow entfernt, `pnpm/action-setup` liest die Version jetzt aus `packageManager` in `package.json`). Nächster geplanter Schritt: **3a (TariffStrategy)**. Blockiert: **3b (SoC-Simulation)** wartet auf Martins Static-Control-Antwort (s. u.); **reale CSV-Formate** warten auf OP#4.
...
- **[GEBAUT: U1] Öffentliche Rechner-Hülle** (`/apps/website`, engine-unabhängig, §9-Paralleltrack): shadcn/ui an DESIGN.md-Tokens gebunden (globals.css = Wahrheit, Bridge-Aliasse), Marketing-Landingpage (mobile-first, animierter Energiefluss), 4-Schritt-Flow (`/rechner`), Report-Grundstruktur (desktop-first, Charts als Platzhalter), Lead-Dialog-Stub. Rendert komplett gegen das **Mock-AnalysisResult** — noch KEINE echte Rechnung. `shared` wird aus dem TS-Source transpiliert (`next.config.mjs → transpilePackages`, Source-Exports in `packages/shared/package.json`). Zwei definierte Andockpunkte für Folge-Prompts:
  - **Prompt 2 (Parser) → `components/flow/step-upload.tsx`:** der `onFile`-Handler nimmt die Datei nur entgegen (PARSE-SLOT-Kommentar). Hier CSV/XLSX-Parsing (§3.2) einhängen, das das getypte `LoadProfile` erzeugt; der Worker-Payload ist dafür bewusst `unknown` (`lib/analysis-protocol.ts`).
  - **Prompt 4 (Engine) → `lib/analysis.worker.ts`:** die Off-Main-Thread-/Progress-Verdrahtung ist real; im Worker sitzt nur eine Mock-Funktion. Nur den Rechen-Block durch den echten Engine-Aufruf ersetzen — Protokoll/Hook (`lib/use-analysis.ts`) bleiben. Der Mock lebt allein in `lib/mock-analysis.ts`.
