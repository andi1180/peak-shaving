...
- **[OFFEN] Einheiten-Konvention der `FinancialParams.*Percent`-Felder** (§3.1/§3.9): Feldname „Percent" vs. Formel `subsidyPercent × totalInvestment` implizieren Anteil (0..1) vs. Prozent (0..100). Contract erzwingt bewusst keinen Upper-Bound; die Festlegung gehört in die Engine (ROI, §3.9) und ist vor belastbaren Amortisationszahlen zu klären.
