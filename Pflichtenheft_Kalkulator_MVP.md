...
| 7 | ~~`[MN]` Bestätigung Arbitrage-Staffelung: tarifbewusstes **Laden** im MVP, preisgetriebenes **Entladen** erst v2~~ **ERLEDIGT, gegen den Code verifiziert (30.07.2026).** §0 (Zeile 22/233) behandelt es bereits als feststehend; der Dispatch (`packages/engine/src/simulation/dispatch.ts`, Schritt 5) lädt tarifbewusst in günstigen Fenstern (`preferChargeInterval`, aus `simulation/tou.ts`), entlädt aber an KEINER Stelle preisgetrieben — Entladung ist ausschließlich Spitzenkappung (Schritt 2) oder Eigenverbrauch begrenzt auf `draw` (Schritt 4, kein Verkauf ins Netz). Kein Code-Pfad für `dynamicPriceProfile`/Spot-Preise (0 Treffer im Engine-Quellcode außerhalb der `[v2]`-Contract-Notiz). | Beide | ~~Dispatch-Scope §3.6/§3.7~~ |
