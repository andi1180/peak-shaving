...
- ⚠ **Offen geblieben:** weist eine Rechnung denselben Posten mit **zwei Sätzen für zwei Gültigkeitszeiträume** aus, verhält sich die Extraktion uneinheitlich (mal `null`, mal der zeitlich letzte). Der System-Prompt sagt zu diesem Fall nichts. Die Regel gehört dort ergänzt und danach an echten Rechnungen nachgemessen; `null` ist bis dahin die sichere Richtung. Ebenfalls ohne Ziel: `rates.arbeitspreisNetzCtPerKwh` — der Netznutzungs-Arbeitspreis hat in Schritt 2 kein Feld, er kommt seit B21-3b aus `public.grid_tariffs`.
